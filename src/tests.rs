...
pub fn obstacle_parser_geo(filepath: &str) ->  Result<Vec<(f64,f64,f64,f64)>, Box<dyn std::error::Error>> {
    let mut lines = BufReader::new(File::open(filepath)?).lines();
    //get the start and goal point first
    let first_row: Vec<f64> = lines.next().expect("Declare more obstacle than there are")?
            .split_whitespace().enumerate().map(|(_, s)| s.parse::<f64>().unwrap()).collect();
    ...
    for _ in 0..obstacle_len {
        ...
        let split: Vec<f64>= line.split_whitespace().enumerate().map(|(_, s)| s.parse::<f64>().unwrap()).collect();
